...
static ssize_t port_link_read_proc(struct file *file, char __user *page, size_t size, 
				loff_t *ppos)
{
	char *ptr = page;
	struct switch_port_link link;
	...
	ptr += sprintf(ptr, "==== switch port link state ====\n");
	
	for (i = 0; i < PORT_NUM; i++) {
		priv_dev->ops->get_port_link(priv_dev, i, &link);
		ptr += sprintf(ptr, "port[%d]=%d\n", i, link.link);
	}
...
static const struct file_operations port_link_ops = {
	.owner = THIS_MODULE,
	.read = port_link_read_proc,
};

static void create_ports_proc_files(void)
{
	port_link = proc_create("switch_port_link", 0666, NULL, &port_link_ops);
	if(port_link) {
		pr_info("create switch proc entry success\n");
	}
}

static void remove_ports_proc_files(void)
{
	remove_proc_entry("switch_port_link", NULL);
}
